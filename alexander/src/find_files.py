...
def dir_files(dirname, criterions, all_needed):
    ...
    '''
    correct_names = []
    full_criterions = fillup_criterions(criterions)
...
def fillup_criterions(criterions):
    '''Change forms of criterions so that they can be passed to
       module re.
       Return criterions in changed form: each one has '^' and '$' added
       respectively in the beginning and in the end.'''
    return ['^' + criterion + '$' for criterion in criterions]
